...
def load():
    rec = (FIX / "reference.txt").read_text(encoding="utf-8")
    ww = json.loads((FIX / "whisper_words.json").read_text(encoding="utf-8"))
    return rec, ww


def by_pair(rows):
    return {(r["reference_spelling"], r["whisper_spelling"]): r for r in rows}
...
def test_sentence_starts_and_matching_tokens_excluded():
    ...
    seen = {r["reference_spelling"] for r in rows} | {r["whisper_spelling"] for r in rows}
...
def pairs(rec, whi, hotwords=HOT):
    ...
    return {(r["reference_spelling"], r["whisper_spelling"]): r for r in nouns.noun_corrections(rec, ww, hotwords)}
...
def test_single_letters_survive_only_against_hotwords():
    ...
    assert rows[("M", "MBA")]["direction"] == "reference→whisper" and rows[("M", "MBA")]["confirmed"] == "yes"
